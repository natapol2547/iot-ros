...
private:
  ...
  // iot-ros: bind the list to libcamera::controls::controls. A default-constructed
  // ControlList has no ControlIdMap, which the IPA serializer encodes as a list of
  // V4L2 controls; an isolated IPA (e.g. an unsigned conda-forge build) then aborts
  // in Camera::start() with "A list of V4L2 controls requires a ControlInfoMap".
  libcamera::ControlList control_values{libcamera::controls::controls};
